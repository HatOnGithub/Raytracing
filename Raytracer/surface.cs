...
namespace Raytracing
{
    public class Surface
    {
        ...
        public void DrawSphere(Vector2 Center, float radius, float MToPx, int color, bool dotted = false)
        {
            int numberOfPixels = dotted ? (int)Math.Ceiling((radius * MToPx) * (2 * MathF.PI)) / 200 : (int)Math.Ceiling((radius * MToPx) * (2 * MathF.PI)); // circumference in radians ( x2 to mitigate gaps in the circle)
